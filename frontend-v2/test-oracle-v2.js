...
async function testOracle() {
  console.log('Testing oracle price fetch for BTC with simplified approach...');
  ...
  try {
    ...
    if (simulated.error) {
      ...
      // Try with string instead
      console.log('\n=== Testing with string ===');
      const stringParam = StellarSdk.xdr.ScVal.scvString('BTC');
      
      const tx2 = new StellarSdk.TransactionBuilder(sourceAccount, {
        fee: '100000',
        networkPassphrase: StellarSdk.Networks.TESTNET,
      })
        .addOperation(contract.call('lastprice', stringParam))
        .setTimeout(30)
        .build();
      ...
      if (simulated2.error) {
        console.log('String error:', simulated2.error);
      } else if ('result' in simulated2 && simulated2.result && simulated2.result.retval) {
        console.log('Success with string!');
        console.log('Result:', simulated2.result.retval);
      }
    } else if ('result' in simulated && simulated.result && simulated.result.retval) {
